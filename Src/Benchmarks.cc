    ...
    bool plotTraffic=0, bool clean=0, double relativeTolerance=1e-5, bool scale=0) 
{
    ...
    // cout << '\t' << "average advantage = "  << Posit32::distillAdvantage() << endl; // has a bug
    ...
    if (general) {
        ...
        // infoFile << '\t' << "average advantage = "  << Posit32::distillAdvantage() << endl; // has a bug
        infoFile.close();
    }
    ...
    // cout << Posit32::distillAdvantage() << endl; // has a bug
}
...
void trisolveTest(Matrix<mpf_class> M, string matrixname="unknown matrix", string identifier="", \
    ...
    cout << "Vector Norm" << M.vectorNorm(bM) << endl;
    cout << '\t' << "double relative residual = "   << rD/M.vectorNorm(bM) << endl;
    cout << '\t' << "float relative residual = "    << rF/M.vectorNorm(bM) << endl;
    cout << '\t' << "posit relative residual = "    << rP/M.vectorNorm(bM) << endl;
   // cout << "Args for found output " << rF.get_d() << endl;
   // cout << "\t next part " << rP.get_d() << endl;
   ...
   // puts("about to distill the advantage");
  //  cout << Posit32::distillAdvantage() << endl; // has a bug
  //  puts("done with distill advantage");
}
...
int main(int argc, char*argv[]) {
    ...
    if (test==0 || test==1 || test==2 || test==4) {
        ...
        puts("finished test identification");
    }
    ...
    {
        case 0:
            puts("selected test 0");
            // need to verify whether there is an args[3] or set a default if there is none.
            if(argc<3) tolerance = "1e-5"; // this is not completely baked, but better than nothing.
            else tolerance   = args[3]; // OK, so this is where the string to MPF is failing.  see what methods we have
            ...
            puts("Check Subarguments");

            if (quire) { identifier += "quire"; puts("quire");} else puts("no quire");
            if (clean) { identifier += "Clean"; puts("clean");} else puts("no clean");
